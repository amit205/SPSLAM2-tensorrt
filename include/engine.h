...
struct Options {
    ...
    std::vector<int32_t> optBatchSizes = {2,4,6};
...
public:
    ...
    bool loadNetwork();
    // Run inference.
    bool runInference(const std::vector<cv::Mat>& inputFaceChips, std::vector<cv::Mat>& featureVectors_descriptor_raw, std::vector<cv::Mat>& featureVectors_descriptor, std::vector<cv::Mat>& featureVectors_detector_logits, std::vector<cv::Mat>& featureVectors_detector, std::vector<cv::Mat>& featureVectors_segmentation);
private:
