...
namespace ORB_SLAM2
{
    ...
    SuperPoint model_engine(options);
    ...
    cv::Mat SPdetect(std::shared_ptr<SuperPoint> model_engine, cv::Mat img, std::vector<cv::KeyPoint>& keypoints, double threshold, bool nms, bool cuda)
    {
        ...
        bool succ = model_engine->runInference(images, featureVectors_descriptor_raw, featureVectors_descriptor, featureVectors_detector_logits, featureVectors_detector, featureVectors_segmentation);
        if (!succ) {
            throw std::runtime_error("Unable to run semanticKeypoints inference.");
        }

        //auto prob = featureVectors_detector[0]; // [H, W]
        //auto desc = featureVectors_descriptor_raw[0];             // [1, H/8, W/8, 256]
        bool use_cuda = cuda && torch::cuda::is_available();
        torch::DeviceType device_type;
        if (use_cuda)
            device_type = torch::kCUDA;
        ...
        //detector_images.push_back(detectorImg);
   
        torch::Tensor prob = torch::from_blob(detectorImg.data, {1, detectorImg.size[0],detectorImg.size[1]}, torch::kF32).to(device);
        torch::Tensor desc = torch::from_blob(descriptorImg_raw.data, {1, descriptorImg_raw.size[0], descriptorImg_raw.size[1], descriptorImg_raw.size[2] }, torch::kF32).to(device);
          
        //torch::Tensor prob = torch::zeros({1, featureVectors_detector[0].size[0], featureVectors_detector[0].size[1]}, torch::kF32).to(device);
        //torch::Tensor desc = torch::zeros({1, featureVectors_descriptor_raw[0].size[0], featureVectors_descriptor_raw[0].size[1], featureVectors_descriptor_raw[0].size[2]}, torch::kF32).to(device);
        prob = prob.set_requires_grad(false);
        desc = desc.set_requires_grad(false);
        //std::memcpy(prob.data_ptr(), featureVectors_detector.data(), sizeof(float)*prob.numel());
        //std::memcpy(desc.data_ptr(), featureVectors_descriptor_raw.data(), sizeof(float)*desc.numel());
        prob = prob.squeeze(0);
    ...
    SPDetector::SPDetector(std::shared_ptr<SuperPoint> _model_engine) : model_engine(_model_engine)
    {
    ...
    void SPDetector::detect(cv::Mat& img, bool cuda)
    {
        ...
        image_scale_width = img.cols / 320.f;
        image_scale_height = img.rows / 240.f;
        ...
        cv::Mat detectorLogits(3, featureVectors_detector_logits[0].size, CV_32F, featureVectors_detector_logits[0].data);
        cv::Mat detectorImg(2, featureVectors_detector[0].size, CV_32FC1, featureVectors_detector[0].data);
        std::cout << detectorImg.size << std::endl;
        std::cout << detectorLogits.size <<std::endl;
        ...
        //detector_logits.push_back(detectorLogits);
        detector_images.push_back(detectorImg);
        mProb = torch::zeros({1, detectorImg.size[0], detectorImg.size[1]}, torch::kF32).to(device);
        //mDesc = torch::zeros({1, featureVectors_descriptor_raw[0].size[0], featureVectors_descriptor_raw[0].size[1], featureVectors_descriptor_raw[0].size[2]}, torch::kF32).to(device);
        mDesc = torch::zeros({1, descriptorImg.size[0], descriptorImg.size[1], descriptorImg.size[2]}, torch::kF32).to(device);
        ...
        std::memcpy(mDesc.data_ptr(), descriptor_images[0].data, sizeof(float)*mDesc.numel());
        ...
        for (height = 0; height < 30; height++) {
            for (width = 0; width < 40; width++) {
             //   for (channel = 0; channel < 1; channel++) {
                    std::cout << mProb[0][height][width] << std::endl;
               // }
                }
            }  */
